#!/usr/bin/env bash
# Launch G2RPO-A math training on the 200-row CL subset, model = Qwen3-0.6B.
# Mirrors run_g2rpoa_math_cl200_gpu23.sh; the only difference is the config
# (which switches model_name_or_path to Qwen3-0.6B and guided_ratio to 0.75
# per paper Table 9, smaller models need stronger guidance).
# GPU layout: visible cuda:0 (= phys 3) trains, visible cuda:1 (= phys 2) runs vLLM.
set -euo pipefail

cd "$(dirname "$0")/.."

RUN_NAME="${RUN_NAME:-g2rpoa_math_cl200_qwen3_0.6b_$(date +%Y%m%d_%H%M%S)}"
GPU_PAIR="${GPU_PAIR:-3,2}"
MAIN_PROCESS_PORT="${MAIN_PROCESS_PORT:-29591}"
CONFIG="${CONFIG:-recipes/config/G2RPO-Atrain_g2rpoa_math_cl200_qwen3_0.6b.yaml}"
...
echo "[$(date '+%F %T')] Starting G2RPO-A math CL200 run on Qwen3-0.6B: ${RUN_NAME}"
...
# PYTHONUNBUFFERED=1 so tqdm/print() flush to train.log line-by-line instead of
# sitting in stdio buffers (which made earlier runs look "frozen" in the log).
env \
...
echo "[$(date '+%F %T')] Finished G2RPO-A math CL200 run on Qwen3-0.6B: ${RUN_NAME}"
