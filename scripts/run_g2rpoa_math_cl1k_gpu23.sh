#!/usr/bin/env bash
# Launch G2RPO-A math training on the curated 1k curriculum-learning subset
# of OpenR1-Math-220k (built by scripts/build_math_cl_subset.py).
# Hyperparams in recipes/config/G2RPO-Atrain_g2rpoa_math_cl1k.yaml.
# GPU layout: visible cuda:0 (= phys 3) trains, visible cuda:1 (= phys 2) runs vLLM.
set -euo pipefail

cd "$(dirname "$0")/.."

RUN_NAME="${RUN_NAME:-g2rpoa_math_cl1k_$(date +%Y%m%d_%H%M%S)}"
GPU_PAIR="${GPU_PAIR:-3,2}"
MAIN_PROCESS_PORT="${MAIN_PROCESS_PORT:-29585}"
CONFIG="${CONFIG:-recipes/config/G2RPO-Atrain_g2rpoa_math_cl1k.yaml}"
...
echo "[$(date '+%F %T')] Starting G2RPO-A math CL1k run: ${RUN_NAME}"
...
echo "[$(date '+%F %T')] Finished G2RPO-A math CL1k run: ${RUN_NAME}"
