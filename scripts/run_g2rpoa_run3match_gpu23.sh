#!/usr/bin/env bash
# Launch G2RPO-A training that mirrors run3 GRPO hyperparams, with
# num_generations=6 and per_device_train_batch_size=6.
# Uses GPUs 2 and 3: visible cuda:0 (= phys 2) trains, visible cuda:1 (= phys 3) runs vLLM.
set -euo pipefail

cd "$(dirname "$0")/.."

RUN_NAME="${RUN_NAME:-g2rpoa_run3match_gpu23_$(date +%Y%m%d_%H%M%S)}"
GPU_PAIR="${GPU_PAIR:-2,3}"
MAIN_PROCESS_PORT="${MAIN_PROCESS_PORT:-29581}"
CONFIG="${CONFIG:-recipes/config/G2RPO-Atrain_g2rpoa_run3match.yaml}"
...
echo "[$(date '+%F %T')] Starting G2RPO-A run3-match run: ${RUN_NAME}"
...
echo "[$(date '+%F %T')] Finished G2RPO-A run3-match run: ${RUN_NAME}"
