...
# (which switches model_name_or_path to Qwen3-1.7B-Base; guided_ratio stays
# at 0.25 per paper Table 9, same alpha as 1.7B post-trained).
# GPU layout: visible cuda:0 (= phys 3) trains, visible cuda:1 (= phys 2) runs vLLM.
set -euo pipefail

cd "$(dirname "$0")/.."

RUN_NAME="${RUN_NAME:-g2rpoa_math_cl200_qwen3_1.7b_base_$(date +%Y%m%d_%H%M%S)}"
...
echo "[$(date '+%F %T')] Starting G2RPO-A math CL200 run on Qwen3-1.7B-Base: ${RUN_NAME}"
...
echo "[$(date '+%F %T')] Finished G2RPO-A math CL200 run on Qwen3-1.7B-Base: ${RUN_NAME}"
