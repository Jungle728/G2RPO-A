#!/usr/bin/env bash
# Launch vanilla GRPO baseline on the 200-row CL math subset, model = Qwen3-1.7B-Base.
# Mirrors run_grpo_math_cl200_baseline_gpu23.sh; the only difference is the
# config (which switches model_name_or_path to Qwen3-1.7B-Base).
# GPU layout: visible cuda:0 (= phys 3) trains, visible cuda:1 (= phys 2) runs vLLM.
set -euo pipefail

cd "$(dirname "$0")/.."

RUN_NAME="${RUN_NAME:-grpo_math_cl200_baseline_qwen3_1.7b_base_$(date +%Y%m%d_%H%M%S)}"
GPU_PAIR="${GPU_PAIR:-3,2}"
MAIN_PROCESS_PORT="${MAIN_PROCESS_PORT:-29597}"
CONFIG="${CONFIG:-recipes/config/G2RPO-Atrain_grpo_math_cl200_baseline_qwen3_1.7b_base.yaml}"
...
echo "[$(date '+%F %T')] Starting GRPO baseline run on Qwen3-1.7B-Base: ${RUN_NAME}"
...
echo "[$(date '+%F %T')] Finished GRPO baseline run on Qwen3-1.7B-Base: ${RUN_NAME}"
