#!/usr/bin/env bash
# Launch G2RPO-A math training on DeepScaleR with run3-style hyperparams,
# num_generations=6, per_device_train_batch_size=6, accuracy+format rewards.
# Uses GPUs 2 and 3: visible cuda:0 (= phys 2) trains, visible cuda:1 (= phys 3) runs vLLM.
# Math has no e2b dependency, so E2B_API_KEY is not required.
set -euo pipefail

cd "$(dirname "$0")/.."

RUN_NAME="${RUN_NAME:-g2rpoa_math_gpu23_$(date +%Y%m%d_%H%M%S)}"
# Order matters: the first visible GPU becomes cuda:0 (trainer), the next is
# cuda:1 (vLLM). 3,2 puts the trainer on physical GPU 3 and vLLM on GPU 2.
GPU_PAIR="${GPU_PAIR:-3,2}"
MAIN_PROCESS_PORT="${MAIN_PROCESS_PORT:-29583}"
CONFIG="${CONFIG:-recipes/config/G2RPO-Atrain_g2rpoa_math.yaml}"
...
echo "[$(date '+%F %T')] Starting G2RPO-A math run: ${RUN_NAME}"
...
echo "[$(date '+%F %T')] Finished G2RPO-A math run: ${RUN_NAME}"
