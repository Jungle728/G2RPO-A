#!/usr/bin/env bash
# Launch vanilla GRPO baseline on the same 200-row CL math subset, mirroring
# the G2RPO-A CL200 run in every dimension except the guidance machinery.
# GPU layout: visible cuda:0 (= phys 3) trains, visible cuda:1 (= phys 2) runs vLLM.
set -euo pipefail

cd "$(dirname "$0")/.."

RUN_NAME="${RUN_NAME:-grpo_math_cl200_baseline_$(date +%Y%m%d_%H%M%S)}"
GPU_PAIR="${GPU_PAIR:-3,2}"
MAIN_PROCESS_PORT="${MAIN_PROCESS_PORT:-29589}"
CONFIG="${CONFIG:-recipes/config/G2RPO-Atrain_grpo_math_cl200_baseline.yaml}"
...
echo "[$(date '+%F %T')] Starting GRPO baseline run: ${RUN_NAME}"
...
echo "[$(date '+%F %T')] Finished GRPO baseline run: ${RUN_NAME}"
