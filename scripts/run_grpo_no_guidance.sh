...
RUN_NAME="${RUN_NAME:-grpo_no_guidance_$(date +%Y%m%d_%H%M%S)}"
GPU_PAIR="${GPU_PAIR:-0,1}"
MAIN_PROCESS_PORT="${MAIN_PROCESS_PORT:-29577}"
MODEL_PATH="${MODEL_PATH:-/share/models/Qwen3/Qwen3-1.7B}"
...
echo "[$(date '+%F %T')] Starting pure GRPO run: ${RUN_NAME}"
...
echo "[$(date '+%F %T')] Finished pure GRPO run: ${RUN_NAME}"
