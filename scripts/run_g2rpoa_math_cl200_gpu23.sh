...
RUN_NAME="${RUN_NAME:-g2rpoa_math_cl200_$(date +%Y%m%d_%H%M%S)}"
GPU_PAIR="${GPU_PAIR:-3,2}"
MAIN_PROCESS_PORT="${MAIN_PROCESS_PORT:-29587}"
CONFIG="${CONFIG:-recipes/config/G2RPO-Atrain_g2rpoa_math_cl200.yaml}"
...
echo "[$(date '+%F %T')] Starting G2RPO-A math CL200 run: ${RUN_NAME}"
...
echo "[$(date '+%F %T')] Finished G2RPO-A math CL200 run: ${RUN_NAME}"
